...
const Wifiplan = () => {
  ...
  const fetchPlans = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:3001/tblwifi");
  ...
  const handleBuy = (wifiid) => {
    window.location.href = `/Wifipaybill?pid=${wifiid}`;
  };
    ...
    <div className="mv-body">
      ...
      <div className="mv-container">
        <h2 className="mv-title">
          <i className="fas fa-wifi"></i> Wifi Plans
        </h2>
        ...
        {!loading && displayPlans.length > 0 ? (
          <table className="mv-table" style={{width:'1200px'}}>
            <thead>
              <tr>
                <th>Plan Name</th>
                <th>Data</th>
