...
const Guestvplans = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3001/tblplan") // replace with your API endpoint
      .then((res) => res.json())
      .then((data) => {
        setPlans(data);
        setLoading(false);
  ...
  const handleLoginRedirect = () => {
    navigate("/Insertlogin"); // redirect to login page
  };
  ...
  return (
    ...
    <div className="gvp-container">
      <h2 className="gvp-title">
        <i className="fa-solid fa-mobile-screen-button"></i> Mobile Plans
      </h2>
