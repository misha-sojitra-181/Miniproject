...
const Wifipaybill = () => {
  ...
  const wifiid = searchParams.get("pid"); // Get planId from query
  ...
  // Fetch logged-in user's mobile on mount
  useEffect(() => {
  ...
  const handleOtpClick = (e) => {
    ...
    if (!otpSent) {
      ...
      if (otpInput === generatedOtp) {
        navigate(`/Userwifipayment?pid=${wifiid}`);
      } else {
        ...
        <div className="card d-flex flex-lg-row flex-column">
          ...
          <div className="col-lg-7 right-panel">
            <form>
              ...
              {/* Display Plan ID */}
              <p>
                <strong>Plan ID:</strong> {wifiid || "Not available"}
              </p>
